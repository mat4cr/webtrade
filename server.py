...
bottle.debug(True)
# Přidán parametr catchup-days="7" pro OTT Navigator
catchup = ' catchup="append" catchup-days="7" catchup-source="?utc={utc}&utcend={utcend}",'
...
def lepsitv_play(id):
    if 'utc' in request.query:
        stream = lepsitv.get_stream(id, str(request.query["utc"]))
    else:
...
def touchtv_playlist():
    ...
    for x,y in touchtv.channels.items():
        t = t + '#EXTINF:-1 provider="Touch TV" tvg-logo="' + y["logo"] + '"' + catchup + y["name"] + "\n" + input_stream + "http://" + client_host  + "/touchtv/" + str(x) + ".m3u8|User-Agent=okhttp/3.12.12\n"
    if t != "":
        t = "#EXTM3U\n" + t
    response.content_type = 'text/plain; charset=UTF-8'
    return t
...
def touchtv_play(id):
    if 'utc' in request.query:
        stream = touchtv.get_catchup(id, str(request.query["utc"]))
    else:
...
def sweet_play(id):
    if 'utc' in request.query:
        stream = sweet.get_catchup(id, request.query["utc"])
    else:
...
def orange_play(id):
    if 'utc' in request.query:
        ...
        stream = orange.get_catchup(id, request.query["utc"], end)
    else:
...
def magio_play(id):
    if 'utc' in request.query:
        ...
        try:
            stream = magio.get_catchup(id, request.query["utc"], str(end))
        except:
...
def tmobile_play(id):
    if 'utc' in request.query:
        ...
        stream = tmobile.get_catchup(id, request.query["utc"], str(end))
    else:
...
def net_play(id):
    if 'utc' in request.query:
        ...
        try:
            stream = net.get_catchup(str(id.split(".")[0]), request.query["utc"], str(end))
        except:
...
def telly_play(id):
    if 'utc' in request.query:
        ...
        try:
            stream = telly.get_catchup(str(id.split(".")[0]), request.query["utc"], str(end))
        except:
...
def rebit_play(id):
    ...
    if 'utc' in request.query:
        ...
        try:
            stream = rebit.get_catchup(ch, request.query["utc"], str(end))
        except:
...
def stvsk_play(id):
    if 'utc' in request.query:
        ...
        stream = stvsk.get_catchup(id, request.query["utc"], str(end))
...
def stvcz_playlist():
    ...
    for x,y in stvcz.channels.items():
        if y["type"] == "tv":
            t = t + '#EXTINF:-1 tvg-id="' + str(x) + '" provider="SledovaniTV.cz" group-title="' + y["group"] + '" tvg-logo="' +y["logo"] + '"' + catchup + y["name"] + "\n" + input_stream + "http://" + client_host  + "/stvcz/" + str(x) + ".m3u8\n"
        else:
            t = t + '#EXTINF:-1 tvg-id="' + str(x) + '" provider="SledovaniTV.cz" radio="true" group-title="' + y["group"] + '" tvg-logo="' +y["logo"] + '"' + catchup + y["name"].replace(" HD", "") + "\nhttp://" + client_host  + "/stvcz/" + str(x) + ".m3u8\n"
    if t != "":
        t = '#EXTM3U x-tvg-url="http://' + client_host + '/stvcz/epg.xml"\n' + t
    response.content_type = 'text/plain; charset=UTF-8'
    return t
...
def stvcz_play(id):
    if 'utc' in request.query:
        ...
        stream = stvcz.get_catchup(id, request.query["utc"], str(end))
...
@route("/stvcz/epg.xml")
def stvcz_epg_route():
    return static_file("epg.xml", root=FILES_DIR, mimetype="application/xml; charset=UTF-8")
...
def kuki_play(id):
    if 'utc' in request.query:
        stream = kuki.get_stream(id.split(".")[0], str(request.query["utc"]))
    else:
...
def o2tvsk_play(id):
    ...
    if 'utc' in request.query:
        ...
        stream = o2tvsk.get_catchup(id, request.query["utc"], end)
    else:
...
def o2tv_play(id):
    ...
    if 'utc' in request.query:
        ...
        stream = o2tv.get_catchup(id, request.query["utc"], end)
    else:
...
def antik_playlist2():
    ...
    for i in r:
        c = i.split("|")
        t = t + '#EXTINF:-1 provider="Antik TV",' + c[0] + "\n" + input_stream + "http://" + client_host  + "/antik/" + str(c[1]) + ".m3u8\n"
    t = "#EXTM3U\n" + t
    response.content_type = 'text/plain; charset=UTF-8'
    return t
...
#EXT-X-ENDLIST'''
    ...
    req = requests.get(url).text.replace("encrypted-file://", "http://" + client_host + "/antik_key/")
    ret=""
